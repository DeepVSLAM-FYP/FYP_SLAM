// webcam_superpoint.cpp
// Based on ip_cam_superpoint.cpp but adapted for USB webcams
...
{
  std::cout << "Usage: " << prog_name << " [options] vocabulary_path settings_path model_path camera_fps target_fps [video_device_number]" << std::endl;
  ...
  std::cout << "Example: " << prog_name << " -t 4 /path/to/vocabulary.fbow /path/to/settings.yaml /path/to/superpoint.xmodel 30 10 0" << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "Note: If video_device_number is not provided, it defaults to 0." << std::endl;
  std::cout << "      Set target_fps to 0 to run as fast as possible (no frame dropping or specific throttling)." << std::endl;
}
...
int main(int argc, char *argv[])
{
  ...
  int camera_fps = 30;
  int target_fps = 10;
  int deviceID = 0; // Default camera device ID
  ...
  if (arg_index + 4 < argc)
  {
    ...
    camera_fps = std::stoi(argv[arg_index + 3]);
    target_fps = std::stoi(argv[arg_index + 4]);
    g_target_fps.store(target_fps); // Initialize the atomic variable
    
    // Get video device number from arguments or use default (0)
    if(arg_index + 5 < argc) { // deviceID is the 6th argument (index 5)
        deviceID = std::stoi(argv[arg_index + 5]);
    }
  }
  else
  {
  ...
  std::cout << "- Camera device: /dev/video" << deviceID << std::endl;
  std::cout << "- Camera FPS: " << camera_fps << std::endl;
  std::cout << "- Target FPS: " << target_fps << std::endl;
  std::cout << "- Output directory: " << output_dir << std::endl;

  cout << "Attempting to open video device: /dev/video" << deviceID << endl;
  cout << "Requested Camera FPS: " << camera_fps << ", Target processing FPS: " << target_fps << endl;
  ...
  try
  {
    ...
    // Start producer thread to capture frames from webcam
    std::thread producer_thread([deviceID, &inputQueue, camera_fps, imageScale, SLAM]()
    {
      // Open the USB webcam
      cv::VideoCapture cap(deviceID);
      if (!cap.isOpened()) {
          cerr << "Error opening webcam at /dev/video" << deviceID << endl;
          cerr << "Available video devices:" << endl;
          // Try a few possible device numbers to help the user
          for(int i = 0; i < 10; i++) {
              cv::VideoCapture testCap(i);
              if(testCap.isOpened()) {
                  cout << "  /dev/video" << i << " (available)" << endl;
                  testCap.release();
              }
          }
          inputQueue.shutdown();
      ...
      cout << "Camera properties: " << width << "x" << height << " @ " << actual_fps << " FPS" << endl;
      
      size_t frame_idx = 0;
      size_t frames_dropped = 0;
      auto last_frame_time = std::chrono::high_resolution_clock::now();
      int target_fps = g_target_fps.load();
      ...
      while (true) {
          ...
          // Enforce target FPS by waiting if needed (if target_fps > 0)
          if (target_fps > 0) {
          ...
          // Calculate timestamp based on frame index and camera FPS
          double timestamp = static_cast<double>(item.index) / camera_fps;
      ...
      inputQueue.shutdown();
      std::cout << "Producer thread finished, processed " << frame_idx 
    ...
    int frame_time_ms = target_fps > 0 ? 1000 / target_fps : 0;
    int wait_time = 0;
    ...
    while (outputQueue.dequeue(result))
    {
      ...
      // Only apply frame timing if target FPS > 0
