...
namespace ORB_SLAM3
{
    ...
    void Tracking::newParameterLoader(Settings *settings)
    {
        ...
        float th_high = settings->thHigh();
        float th_low = settings->thLow();
    ...
    // This is not  used anymore, but we keep it for backward compatibility
    ...
    Sophus::SE3f Tracking::GrabImageMonocular(const ResultQueueItem &Input)
    {
        ...
        mCurrentFrame.mNameFile = Input.filename;
        mCurrentFrame.mnDataset = mnNumDataset;
        ...
        int iniThFAST, int minThFAST, const std::string &type)
    {
        ...
        else if (type == "DUMMY")
        {
            ...
            std::cout << "Feature extractor not implemented using ORB" << std::endl;
            // return new ORBextractor(nFeatures, scaleFactor, nLevels, iniThFAST, minThFAST);
            return new SIFTextractor(nFeatures);
        }
        else
...
bool Tracking::TrackWithMotionModel()
{
        ...
        {
            std::cout << "[DEBUG][Track] Frame ID: " << mCurrentFrame.mnId << "  [TrackWithMotionModel]  START" << std::endl;
        }
#endif
    ORBmatcher matcher(0.9, GlobalFeatureExtractorInfo::GetFeatureExtractorType() == "ORB" || GlobalFeatureExtractorInfo::GetFeatureExtractorType() == "SIFT" );
        ...
        else
        {
            ...
            if(std::getenv("DEBUG_TrackWithMMVel") != nullptr)
            {
                std::cout << "[DEBUG][Track] Frame ID: " << mCurrentFrame.mnId << "  Using velocity model prediction" 
                        ...
                        << "  mCurrentFrame.GetPose(): " << mCurrentFrame.GetPose().matrix() << std::endl;
            }
#endif
        }
    ...
    bool Tracking::TrackLocalMap()
    {
        ...
        if (mSensor == System::IMU_MONOCULAR)
        {
            ...
            if(std::getenv("DEBUG_TrackLM") != nullptr)
            {
                std::cout << "[DEBUG][TrackLM] Frame ID: " << mCurrentFrame.mnId << "  IMU_MONOCULAR" << std::endl;
            }
#endif
                ...
                if (bTcw)
                {
                    ...
                    for (int j = 0; j < np; j++)
                    {
                        if (vbInliers[j])
                        {
                            ...
                            mCurrentFrame.mvpMapPoints[j] = static_cast<MapPoint *>(NULL);
                    }
