...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::MONOCULAR)
    {
        ...
        {
            if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
                mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor/* This is the same ORB extractor with 5 x nfeatures*/,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
            else
                mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft/* This is the same ORB extractor with nfeatures*/,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
        }
    }
...
void Tracking::Track()
{
    ...
    //************************************Initializes the map if it is not initialized 
